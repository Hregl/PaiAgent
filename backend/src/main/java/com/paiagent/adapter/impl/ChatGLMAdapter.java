...
/**
 * ChatGLM (智谱) adapter - OpenAI compatible API format.
 */
public class ChatGLMAdapter implements LLMAdapter {
    ...
    public ChatGLMAdapter(String apiKey, String baseUrl) {
        this.apiKey = apiKey;
        this.baseUrl = baseUrl.isEmpty() ? "https://open.bigmodel.cn/api/paas/v4" : baseUrl;
    ...
    public String chat(String prompt, Map<String, Object> config) throws Exception {
        String model = (String) config.getOrDefault("model", "glm-4-flash");
        ...
        try (Response response = client.newCall(request).execute()) {
            if (!response.isSuccessful()) {
                ...
                throw new RuntimeException("ChatGLM API error: " + response.code() + " - " + errorBody);
            }
