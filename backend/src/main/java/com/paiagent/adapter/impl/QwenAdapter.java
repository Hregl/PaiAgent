...
/**
 * Qwen (通义千问) adapter - DashScope OpenAI compatible mode.
 */
public class QwenAdapter implements LLMAdapter {
    ...
    public QwenAdapter(String apiKey, String baseUrl) {
        this.apiKey = apiKey;
        this.baseUrl = baseUrl.isEmpty() ? "https://dashscope.aliyuncs.com/compatible-mode/v1" : baseUrl;
    ...
    public String chat(String prompt, Map<String, Object> config) throws Exception {
        String model = (String) config.getOrDefault("model", "qwen-turbo");
        ...
        String requestBody = mapper.writeValueAsString(Map.of(
                "model", model.isEmpty() ? "qwen-turbo" : model,
        ...
        try (Response response = client.newCall(request).execute()) {
            if (!response.isSuccessful()) {
                ...
                throw new RuntimeException("Qwen API error: " + response.code() + " - " + errorBody);
            }
